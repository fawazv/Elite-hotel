...
export async function initUserEventConsumer() {
  ...
  const queue = 'notificationService.user.events'
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      ...
      console.error('[NotificationService] User event processing error:', error)
      channel.ack(msg)
    }
  })

  console.log('[NotificationService] User event consumer started')
}
