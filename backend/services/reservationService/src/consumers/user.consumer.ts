...
export async function initUserEventConsumer() {
  ...
  const queue = 'reservationService.user.events'
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      ...
      logger.error('[ReservationService] User event processing error:', error)
      channel.ack(msg)
    }
  })

  logger.info('[ReservationService] User event consumer started')
}
